                ...
                if len(bdinfo_keys) > 1:
                    ...
                    for i, key in enumerate(bdinfo_keys[1:], start=1):  # Skip the first bdinfo
                        ...
                        if new_images_key in meta and meta[new_images_key]:
                            ...
                            meta['retry_count'] += 1
                            meta[new_images_key] = []
                            new_screens = glob.glob(f"PLAYLIST_{i}-*.png", root_dir=f"{meta['base_dir']}/tmp/{meta['uuid']}")
                            if not new_screens:
                    ...
                    if each['type'] == "BDMV":
                        ...
                        if i == 0:
                            ...
                            if new_images_key in meta and meta[new_images_key]:
                                ...
                                meta['retry_count'] += 1
                                meta[new_images_key] = []
                                new_screens = glob.glob(f"{meta['discs'][i]['name']}-*.png", root_dir=f"{meta['base_dir']}/tmp/{meta['uuid']}")
                                if not new_screens:
                                    try:
